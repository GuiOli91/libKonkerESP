...
bool testHTTPSubscribeConn(){
    ...
    Serial.println("[testHTTPSubscribeConn] Sending HTTP");
    ...
    Serial.println("[testHTTPSubscribeConn] Trying to connect");
    ...
    Serial.println("[testHTTPSubscribeConn] Testing HTTP subscribe to: " + String(buffer));
    //Serial.print(">");
...
bool pubHttp(char const channel[], char const msg[], int* ret_code){
    ...
    Serial.println("Publishing to " + String(buffer) + "; Body: " + String(msg));
    //Serial.print(">");
    http.end();   //Close connection
    ...
    *ret_code = httpCode;
    ...
    if (!pubCode){
        Serial.println("PUB request failed");
        Serial.println("");
        failedComm=1;

        return 0;
    }else{
        Serial.println("PUB request sucess");
        Serial.println("");
...
bool subHttp(char const channel[],CHANNEL_CALLBACK_SIGNATURE){
    ...
    if (!subCode){
        Serial.println("SUB request failed");
        Serial.println("");
        failedComm=1;
    }else{
        Serial.println("SUB request sucess");
        Serial.println("");
        String strPayload = http.getString();
        //Serial.println("strPayload=" + strPayload);
        ...
        if (strPayload!="[]"){
            ...
            Serial.println("Calling SUB callback");
